...
export default function RevistasHeroSection({ hero }) {
  return (
    <section className="bg-[#001d3d] pt-40 pb-28 px-6 relative overflow-hidden border-b-[8px] border-[#ea580c]">
      <div className="absolute inset-0 z-0">
        <img
          src={hero.imageUrl}
          alt="Revistas y Libros"
          className="w-full h-full object-cover grayscale opacity-[0.15] contrast-[1.2] mix-blend-screen"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#001d3d] via-[#001d3dcc] to-transparent" />
      </div>
      ...
      <div className="max-w-[1000px] mx-auto relative z-10 text-center">
        <div className="inline-flex items-center gap-3 bg-white/10 border border-white/20 px-6 py-2.5 rounded-xl mb-6 backdrop-blur-md shadow-sm">
          <Library className="text-[#ea580c]" size={16} />
        ...
        <p className="text-lg md:text-xl text-blue-200/90 font-medium italic max-w-2xl border-l-4 border-[#ea580c] pl-6 py-2 leading-relaxed mx-auto text-left md:text-center md:border-l-0 md:pl-0">
          {hero.description}
        </p>
      </div>
    </section>
