...
import { HardDrive, Sigma } from 'lucide-react';
import StatisticalOverlay from '../../atoms/resources/StatisticalOverlay';

export default function DocumentosHeroSection({ hero }) {
  return (
    <section className="bg-[#001d3d] pt-40 pb-20 px-6 relative overflow-hidden border-b-[8px] border-[#ea580c]">
      ...
      <Sigma className="absolute -right-10 top-20 text-white/5 opacity-40" size={350} />
      
      <div className="max-w-[1000px] mx-auto relative z-10 text-center">
        <div className="inline-flex items-center gap-3 bg-white/10 border border-white/20 px-6 py-2.5 rounded-xl mb-6 backdrop-blur-md">
          <HardDrive className="text-[#ea580c]" size={16} />
        ...
        <h1 className="text-5xl md:text-7xl font-black text-white uppercase tracking-tighter leading-none mb-6">
          {hero.title[0]} <br/> 
          <span className="text-[#ea580c] uppercase font-serif tracking-normal">{hero.title[1]}</span>
        </h1>
