function AutofinancedCurriculumSection({ meta, modules, activeMod, onSelectMod }) {
          ...
          <div className="overflow-hidden rounded-[2.5rem] bg-white shadow-2xl">
            <div className="flex flex-col items-center justify-between gap-4 border-b border-slate-100 bg-slate-50 p-8 md:flex-row">
              ...
              <div className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm sm:text-base md:text-lg font-black uppercase tracking-widest text-slate-400 shadow-sm">
                Cursos por módulo: <span className="ml-2 text-lg text-[#001d3d]">{currentModule.items.length}</span>
              </div>
            </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-white border-b border-slate-100">
                    ...
                    <th className="px-6 py-5 text-sm sm:text-base md:text-lg font-black uppercase tracking-widest text-slate-400">Asignatura</th>
                    <th className="px-6 py-5 text-sm sm:text-base md:text-lg font-black uppercase tracking-widest text-slate-400 text-center">Tipo</th>
                  </tr>
                </thead>
                  ...
                  {currentModule.items.map((item) => (
                    <tr key={item.id} className={`transition-colors ${item.highlight ? 'bg-orange-50/50 hover:bg-orange-50' : 'hover:bg-slate-50'}`}>
                      ...
                      <td className="px-6 py-5 text-center">
                        <span className={`rounded-full border px-3 py-1 text-base sm:text-base md:text-lg font-black uppercase tracking-widest ${item.highlight ? 'border-[#ea580c]/20 bg-[#ea580c]/10 text-[#ea580c]' : 'border-[#001d3d]/10 bg-[#001d3d]/5 text-[#001d3d]'}`}>
                          {item.type}
                        </span>
                      </td>
                    </tr>
...
export default AutofinancedCurriculumSection
